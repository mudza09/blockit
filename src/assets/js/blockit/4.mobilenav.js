...
function mobilenavConfig(options) {
    ...
    if (mainNav !== null) {    
        ...
        function buttonUrl(index) {
            let urlValue;
            this.options.buttons[index].url.length > 0 
                ? urlValue = this.options.buttons[index].url 
                : urlValue = signinEle.querySelector('a').href; 
            return urlValue;
        }
        ...
        function buttonName(index) {
            let nameValue;
            this.options.buttons[index].name.length > 0 
                ? nameValue = this.options.buttons[index].name 
        ...
        function buttonIcon(index) {
            let iconTag;
            this.options.buttons[index].icon !== undefined && this.options.buttons[index].icon.length > 0
                ? iconTag = `<i class="fas fa-${this.options.buttons[index].icon} uk-margin-small-left"></i>`
                : iconTag = '';
            return iconTag;
        }
        ...
        if (this.options.extraButton && signinEle !== null && signinEle.children.length > 0) {
            this.options.buttons.forEach(function(e, i) {
                signinBtn += `<a href="${buttonUrl(i)}" class="uk-button uk-button-${this.options.buttons[i].type} uk-border-rounded uk-align-center" style="margin-bottom : -12px">${buttonName(i)}${buttonIcon(i)}</a>`;
            })
        }
        ...
        mainNav.parentElement.removeChild(mainNav.parentElement.lastChild);
        mainNav.parentElement.appendChild(mobileBtn).appendChild(modalFull);
    }
};
mobilenavConfig({});